...
def get_tokenizer(args):
    tokenizer = AutoTokenizer.from_pretrained('/run/user/guankaisi/openbmb/cpm_10b',trust_remote_code=True)
    return tokenizer
...
def get_model(args):
    ...
    if args.use_delta:
        ...
        delta_state = torch.load('cpm_finetune/cpm-bee-10b-delta.pt')
        delta_model.log()
...
def finetune(
    ...
    # 开始训练
    train_state = torch.load('cpm_finetune/cpm-bee-10b_2-train_state.pt')
    epoch0 = train_state['epoch']
    iteration_0 = train_state['iteration']

    # optimizer.load_state_dict(torch.load('cpm_finetune/cpm-bee-10b_2-opt_state.pt'))
    # lr_scheduler.load_state_dict(torch.load('cpm_finetune/cpm-bee-10b_2-lr_state.pt'))
