...
class CSTSEval(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('\n\n***** Transfer task : Chinese-STSBenchmark*****\n\n')
        self.seed = seed
        self.samples = []
        train = self.loadFile(os.path.join(task_path, 'cnsd-sts-train.txt'))
        dev = self.loadFile(os.path.join(task_path, 'cnsd-sts-dev.txt'))
        test = self.loadFile(os.path.join(task_path, 'cnsd-sts-test.txt'))
        self.datasets = ['train', 'dev', 'test']
        self.data = {'train': train, 'dev': dev, 'test': test}
